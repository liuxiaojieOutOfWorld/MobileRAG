...
def get_action_prompt(instruction, clickable_infos, width, height, keyboard, thought_history, summary_history, action_history, memory, add_info, decision, current_task="", complete_task="", memrag = ""):
    ...
    if len(thought_history) > 1:
        ...
        prompt += "### App-Specific Guidelines ###\n"
        prompt += "Spotify App:\n"
        prompt += "   - You can tap the '+' icon to add music.\n"
        prompt += "   - You can tap '✅' to add music in specific playlist."
        prompt += "Prime video App:\n"
        prompt += "   - if you have added a movie to your watchlist, the 'Watchlist' icon will turn '+' into '✅'\n"
        prompt += "Notes App:\n"
        prompt += "   - After opening Notes app, ALWAYS look for and tap the '+' button or blue icon to create a new note\n"
        prompt += "   - The '+' button or blue icon with pen is usually located in the bottom-right corner\n"
        prompt += "   - CRITICAL: You MUST first tap the blue pencil icon in the bottom-right corner to create a new note\n"
        prompt += "   - DO NOT try to tap on input boxes or text areas directly - they will not activate the keyboard\n"
        prompt += "   - ONLY the blue pencil icon in the bottom-right corner will create a new note and activate the keyboard\n"
        prompt += "   - The blue pencil icon creates a new text file (.txt) where you can write content\n"
        prompt += "   - Only after creating a new note will the keyboard appear and allow you to type text\n"
        prompt += "   - If no '+' button is visible, try swiping down to refresh the notes list\n"
        prompt += "   - You can stop after typing, needn't to find save icon\n"
        prompt += "   - Once a new note is created, you can start typing\n\n"
        # prompt += "Phone power and network speed :\n"
        # prompt += "   - On the home screen or in the status bar, you can often swipe down(Swipe (500, 1800), (500, 1400)) to view the current phone battery level and network(Internet) speed(tap internet and then tap setting icon to see internet detail"
        # prompt += "X app:\n"
        # prompt += "   - The '+' button means create a post instead of a comment\n\n"
        # prompt += "Search for something:\n"
        # prompt += "   - If there is a search bar on the page, clicking the search bar and entering your query is the most effective way to find information about an item, place, or product.\n"
        # prompt += "   - Tap search bar and type the keyword you want to know is the best way to find information.\n"
        # prompt += "Review Finding Guidelines:\n"
        # prompt += "   - To find reviews for a restaurant: First tap to enter the restaurant page\n"
        # prompt += "   - Then look for and tap the 'Reviews' or 'See all reviews' button\n"
        # prompt += "   - If no review button is visible, swipe down the page to load more reviews\n"
        # prompt += "   - Continue swiping down to see more reviews until you find detailed dish recommendations\n"
        # prompt += "   - Look for reviews that mention specific dishes, 'must-try', 'recommended', or food names\n\n"
        # prompt += "When planning the itinerary, you MUST perform separate searches for attractions and for dining options:\n"
        # prompt += "- First, search for and select the top attractions in the target city (such as museums, parks, or landmarks) according to the user's preferences.\n"
        # prompt += "- After completing the attractions search, perform a separate search for the best dining options (such as restaurants or cafes), again considering the user's preferences (for example, avoid seafood if requested).\n"
        # prompt += "For each category (attractions and dining), you should:\n"
        # prompt += "  • Use the appropriate app or search function to find relevant information.\n"
        # prompt += "  • Carefully review the results and select the most suitable options.\n"
        # prompt += "  • Record your findings for each category before moving on to the next.\n"
        # prompt += "  • You only need to select ONE suitable option for each category. Do NOT spend time comparing multiple options or listing many candidates—just pick one that meets the user's requirements and proceed.\n"
        # prompt += "You may need to switch between different apps or pages to complete both searches.\n"
        # prompt += "**Do NOT try to find both attractions and dining in a single search step.**\n"
